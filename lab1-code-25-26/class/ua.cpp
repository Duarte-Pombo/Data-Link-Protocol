...
Duarte Martins <duartepombomartins@gmail.com>
Anexos
terça, 23/09, 15:56 (há 2 dias)
para up202206640@g.uporto.pt, mim


 Um anexo
  •  Verificado pelo Gmail
...
int main(int argc, char *argv[])
{
    ...
    while (STOP == FALSE)
    {
        ...
        for (int i = 0; i<32; i++){
            bb[i] = byte;
            if (byte = 0x03){
                printf ("received message, sending confirmation\n");

                //validate info received buf[1] ^ buf[2] == buf[3]
                if (bb[2]^bb[1] == bb[3]){
